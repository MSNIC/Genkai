...
export const meta = { tags: ['voice-rooms'], requireCredential: true, kind: 'write:account', errors: voiceRoomErrors, res: participantSchema } as const;
...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(private voiceRoomService: VoiceRoomService, private globalEventService: GlobalEventService) {
		super(meta, paramDef, async (ps, me) => {
			try {
				const participant = await this.voiceRoomService.join(ps.roomId, me.id);
				this.globalEventService.publishVoiceRoomStream(ps.roomId, 'participantChanged', { userId: me.id });
				return { id: participant.id, roomId: participant.roomId, userId: participant.userId, role: participant.role };
